...
//@TODO gÃ©rer le passage en mode plein Ã©cran lorsqu'une formule est en cours d'Ã©dition.

(function() {
    var Event = tinymce.dom.Event;
    //Les diffÃ©rentes classes utilisÃ©es pour le rendu et autre ...
    ...
    tinymce.create('tinymce.plugins.MathjaxPlugin', {
        ...
        init : function(ed, url) {
            ...
            t.mathBox = null;//div de prÃ©-visualisation des formules en cours d'Ã©dition
            t.completion = {//est utile popur la complÃ©tion de commande via ctrl+space
                ideb: -1, //indice de dÃ©but pour la commande en cours dans texCommands
                iend: 1, //fin de la selection courante
                dec: 0,//lorsqu'on fait ctrl+space plusieurs fois de suite, indique la nouvelle commande Ã  afficher
                nc: 2 //Pour gÃ©rer les 2 nodeChange produit par le raccourci ctrl+space
            };
            t.fullscreenInit = false;
            t.actif = false;
            t.parent = null;//sert Ã  gÃ©rer les aller-retour en mode plein Ã©cran
            ...
            ed.onPreInit.add(function(ed){               
                ...
                //Si l'Ã©diteur actuel est celui du mode plein Ã©cran (fullscreen)
                if(ed.id.indexOf('fullscreen') != -1) {
                    t.fullscreenInit = true;
                    //parent est une rÃ©fÃ©rence vers l'instance du plugin mathjax de l'Ã©diteur parent.
            ...
            // Lorsque le document est chargÃ© dans l'Ã©diteur 
            //ed.onLoadContent.add(function(ed,o){console.log(o)});
            //Avant que le document ne soit sÃ©rialisÃ© ; il est encore temps d'agir sur celui-ci : ed.onPreProcess.add(function(ed,o){});
            
            // aprÃ¨s la sÃ©rialisation du document ... 
            // o.content la chaine obtenue ; o.save vaut true si le formulaire qui contient l'Ã©diteur est soumis au serveur.
            // o.source_view vaut true si l'utilisateur a appuyÃ© sur le bouton "html".
            ed.onPostProcess.add(function(ed,o){
                if(o.save || o.source_view) o.content = t._cleanMath(o.content);//sert Ã  Ã©liminer le contenu spÃ©cifique produit par MathJax.
            });
            
            //Avant que l'Ã©criture dans le corp du document 
            //o.initial : si le document est chargÃ© pour la premiÃ¨re fois ; o.source_view : si les sources sont mises Ã  jour dans la fenÃªtre html
            ed.onBeforeSetContent.add(function(ed,o){
                o.format = "raw";//empÃªche le formatage de tinymce
                if(o.initial || o.source_view) o.content = t._marqueMath(o.content);
                //le code source du document est modifiÃ©, on se prÃ©pare Ã  Â«recompilerÂ» les formules
            ...
            //Lorsqu'on revient du mode plein Ã©cran, on met Ã  jour l'ID du parent
            ...
            //@TODO (webkit) : il faudrait veiller Ã  ce que 1. il soit impossible de supprimer les $ 2. lorsque le curseur dÃ©passe le dernier dollar, on simule une sortie ...
            ed.onNodeChange.add(function(ed, cm, n, c) {//cm->controlManager, n == ed.selection.getNode(), c-> collapsed ?
                ...
                if(cmp.nc) {//pour "annuler" les 2(?) nodeChange qui suivent un ctrl+espace (commande de complÃ©tion) ... la touche ctrl dÃ©clenche un nodeChange
                    cmp.nc--;
                    return;
                } else if (cmp.ideb != -1)   {//rÃ©initialisation de l'objet Â«completionÂ»
                    cmp.ideb = -1;
                    cmp.iend = 1;
                    cmp.dec = 0;
                }
                ...
                //on dÃ©sactive les controls lorsqu'on est dans une formule (la commande undo n'est pas dÃ©sactivÃ©e !)
                //@TODO : trouver un moyen de dÃ©sactiver la commande undo lors de la saisie d'une formule
                if(math) each(cm.controls,function(v,k){
                    v.setDisabled(1);
                });
                
                //pas de formule Ã  traduire ; hors maths ou dans une formule en cours d'Ã©dition ... => rien Ã  faire !
                if(!math && !t.mathedit) return;
                //nous entrons dans une formule interprÃ©tÃ©e ...
                else if(math && dom.hasClass(math,cmjv)) {
                    // Si il reste une formule Ã  interprÃ©ter qui n'est pas celle-ci, on s'en occupe
                ...
                //on sort juste d'une formule en cours d'Ã©dition
            ...
            //gestion de la complÃ©tion de commande.
            ed.onKeyDown.add(function(ed,ev){
                ...
                if (ev.ctrlKey && ev.which == 32) {//Ctrl+espace
                    ...
                    if((math = t._ismath(n)) && ed.dom.hasClass(math,cmje)){
                        ...
                        if(cmp.ideb == -1){//ctrl+espace n'a pas Ã©tÃ© tapÃ© 2 fois de suite ou rien n'avait Ã©tÃ© trouvÃ© lors de la prÃ©cÃ©dente recherche
                ...
                else if(!(ev.metaKey || ev.ctrlKey || ev.altKey || ev.shiftKey) && t.completion.ideb != -1) {//rÃ©initialisation de l'objet Â«completionÂ»
                    t.completion.ideb = -1;
                    t.completion.iend = 1;
                    t.completion.dec = 0;
                } 
            ...
            ed.onKeyPress.add(function(ed,ev){
                ...
                //console.log(ev + " :" + key);
                if (key == '$' && !ev.ctrlKey) {
                    Event.cancel(ev);//empÃªche l'insertion ordinaire de '$'
                    each(ed.controlManager.controls,function(v,k){
                        v.setDisabled(1)
                    });
                    math = t._ismath(n);
                    if(math) {//si on est dÃ©jÃ  dans une formule, on change de mode : mode en ligne -> mode block ou le contraire.
                    ...
                    //On insÃ¨re une formule en Ã©dition et on met le curseur entre les $$ (?).
                    ...
                    //if(tinymce.isGecko && ev.charCode == 0) return;//la touche "flÃ¨che vers le bas" a un keyCode de 40 qui donne key == "(" !; son charCode est nul.
                ...
                else if((ev.charCode >= 65 && ev.charCode <= 90 || ev.charCode >=97 && ev.charCode <=122)//afin de visualiser la partie du tableau TexCommands adÃ©quate.
        ...
        _globalMathTypeset: function(mathID, spanID){
            ...
            //le curseur est sur une formule, on le dÃ©place sur le noeud suivant.
        ...
        _mathTypeset : function(math){
            ...
            TEX = t._norm(TEX);
            if(tinymce.isWebKit) TEX = TEX.substring(1,TEX.length-1); //on enlÃ¨ve les $$.(bug wekkit)
            //on gÃ¨re le curseur si on entre directement dans une autre formule ?
        ...
        //Cette mÃ©thode sert Ã  marquer les formules (qui ne le seraient pas ...).
        //Elle est appelÃ©e Ã  deux reprises : 1. lorsque l'Ã©diteur est chargÃ© 2. Lorsque l'utilisateur met Ã  jour le code html via l'Ã©diteur de code
        _marqueMath : function(content){
            var text = this._cleanMath(content);
            //on nettoie les rÃ©sidues avant toute chose ...
            var newtext = ""
            //le motif qui suit dÃ©tecte les formules qui sont marquÃ©es ou non
            ...
            //                match[0] : //la formule est dÃ©ja marquÃ©e ; on ne modifie rien
        ...
        //sert Ã  nettoyer le contenu de l'Ã©diteur des traces laissÃ©es par le plugin.
        _cleanMath : function(content){
            function rep(re, str) {
                content = content.replace(re, str);
            };
            //les deux lignes qui suivent servent Ã  Ã©liminer le html relatif aux formules en cours aprÃ¨s que tinymce ait sÃ©rializer le document en mode "html"
            rep(/<span[^>]*?mcemathinline mcemathv[^>]*?>.*?\n.*?\n(.*?)\n.*?\n<\/span>/g,"\\($1\\)");
            rep(/<span[^>]*?mcemathdisplaystyle mcemathv[^>]*?>.*?\n.*?\n(.*?)\n.*?\n<\/span>/g,"\\[$1\\]");
            //pour Ã©liminer le html aprÃ¨s une sÃ©rialisation en mode "raw"
            rep(/<span[^>]*?mcemathinline mcemathv[^>]*?>.*?<script[^>]*?>(.*?)<\/script><\/span>/g,"\\($1\\)");
            rep(/<span[^>]*?mcemathdisplaystyle mcemathv[^>]*?>.*?<script[^>]*?>(.*?)<\/script><\/span>/g,"\\[$1\\]");
            //on Ã©limine les marqueurs de formules
        ...
        //MathJax pollue le document (body) avec trois div (deux au dÃ©but, la derniÃ¨re tout Ã  la fin)
        ...
        _selection: function(node){
            ...
            if(!child) {//une nouvelle formule vient d'Ãªtre insÃ©rÃ©e.
        ...
        //est appelÃ©e lorsqu'on sort d'une formule.
        _removeMathHandler: function(){
            ...
            t.doc.removeEventListener("keyup",t.mathViewHandler,false);//on supprime l'Ã©couteur dÃ©finit dans _mathPreview.
            each(cm.controls,function(v,k){//rÃ©activation des controles de l'Ã©diteur.
                v.setDisabled(0)
            });
            t.mathViewHandler = null;
        },
        
        //gÃ¨re la prÃ©visulation des formules (ainsi que les $ dans webkit ...)
        _mathPreview: function(math){
            ...
            //lorsqu'on est dans une formule, la prÃ©visualisation est mis Ã  jour grÃ¢ce Ã  cet Ã©couteur
            t.mathViewHandler = function(ev){
                if (ed.selection.getStart() === math) {
                    if(tinymce.isWebKit){//on essaie de gÃ©rer webkit au mieux ...
                        var rng = ed.selection.getRng();
            ...
            //ajout de l'Ã©couteur dÃ©fini plus tÃ´t ; on l'enlÃ¨ve dÃ¨s que l'utilisateur sort d'une formule (cf _removeMathHandler).
        ...
        //lors d'un setContent ou d'un getContent avec o.format = "html", tinymce pollue les formules affichÃ©es qui ne sont alors plus Ã©ditables.
        //1Ã¨re solution : forcer le format o.format = "raw" lors de ces actions.
        //2Ã¨me solution (serait plus propre ...) : agir sur le schÃ©ma.
        _testSchemaSettings : function(){
            var ed = this.editor, sr = ed.schema.getElementRule('span');
            sr.removeEmpty = false;
            sr.attributes["aria-role"] = {};//impossible d'empÃªcher cet attribut d'Ãªtre Ã©liminer.
            sr.attributes["role"] = {};
            ...
            //@TODO : tinymce commente les scripts <![CDATA[...]]!>, il faudrait empÃªcher cela (pas fastoche)
        ...
        //insÃ¨re un espace autour des inÃ©galitÃ©s strictes 
        _norm : function(str){
            var text = str;
        ...
        _mathjaxInit : function(url){
            ...
            //On charge la feuille de style qui gÃ¨re le rendu des formules en cours d'Ã©dition
            ed.dom.loadCSS(url+'/css/mathjax.css');
            
            //On insÃ¨re mathjax dans le head du document Ã  Ã©diter.
            
            //infos de configuration pour MathJax
            var script1 = dom.create('script',{
                type:"text/x-mathjax-config"
            },t.config);
            //Chargement du script principal via le CDN (et si jamais le CDN de MathJax Ã©tait indisponible ?)
            ...
            //MathJax peut mettre un certain temps avant d'Ãªtre prÃªt ...
            var timeId = win.setInterval(function(){
                if(win.MathJax && win.MathJax.Hub.queue) {
                    ...
                    //lorsqu'on est en mode plein Ã©cran
                    if(t.parent && t.parent.actif) {
                        //sert Ã  mettre Ã  jour le compteur ID de MathJax.ElementJax ; 
                        //Ã  ne faire qu'aprÃ¨s s'Ãªtre occupÃ© de la boite de prÃ©visualisation (ainsi son ID sera 1)
                        t.actif = true;
                        t.mathBox = null;
                    ...
                    //l'initialisation est achevÃ©e, inutile de la rÃ©pÃ©ter ...

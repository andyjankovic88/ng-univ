...
var Grid = fc.Grid = RowRenderer.extend({
	...
	headCellHtml: function(cell) {
		var view = this.view;
		var date = cell.start;
		var isCurrentDate = view.calendar.getNow().isSame(date, 'day') ? 'class="current"' : '';

		return '' +
			'<th class="fc-day-header ' + view.widgetHeaderClass + ' fc-' + dayIDs[date.day()] + '">' +
				htmlEscape(date.format(this.colHeadFormat)) + '<div ' + isCurrentDate + '><span class="vert-helper"></span><span>' + date.date() + '</span></div>'
			'</th>';
	},
...
var TimeGrid = Grid.extend({
	...
	slatRowHtml: function() {
		...
		while (slotTime < this.maxTime) {
			...
			noonClass = ((slotDate.hours() == 12) && (minutes == 0)) ? 'class="noon"' : '';


			axisHtml =
				'<td class="fc-axis fc-time ' + view.widgetContentClass + '" ' + view.axisStyleAttr() + '>' +
					((!slotNormal || !minutes) ? // if irregular slot duration, or on the hour, then display the time
						'<span ' + noonClass + '>' + // for matchCellWidths
